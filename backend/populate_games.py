...
for week_number in range(1, REG_SEASON_WEEKS + 1):
    ...
    if response.status_code == 200:
        ...
        if not game_day_tables:
            error_message = (
                "Error: No game day tables found. Check the web page structure."
            )
            raise ValueError(error_message)

        for table in game_day_tables:
            date_string = table.find(
                "h4", class_=html_classes["date_class"]
            ).text.strip()
            if date_string is None:
                error_message = (
                    "Error: Game date not found. Check the web page structure."
                )
                raise ValueError(error_message)
            ...
            if rows is None:
                error_message = (
                    "Error: No game rows found. Check the web page structure."
                )
                raise ValueError(error_message)
            ...
            for row in rows:
                ...
                away_team_city = away_cell.find(
                    "span", class_=html_classes["team-name"]
                ).text
                home_team_city = home_cell.find(
                    "span", class_=html_classes["team-name"]
                ).text
                ...
                # convert them to team abbreviations
